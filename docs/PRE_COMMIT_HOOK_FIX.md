# Исправление pre-commit: не считать статьи блога «запрещёнными»

## Что произошло (03.02.2026)

В pre-commit хуке (`.git/hooks/pre-commit`) список запрещённых паттернов включал **STRATEGY**. При коммите с добавлением статьи **oracle-strategy** (RU/EN/ES) хук сработал на путь `src/content/.../oracle-strategy/`, посчитал эти файлы запрещёнными, **снял их со staging** и выполнил перенаправление. В итоге в коммит попали все остальные изменения, но **не попала сама статья ORACLE 1.1**.

## Исправление

В `.git/hooks/pre-commit` при формировании списка `BLOCKED_FILES` нужно **исключить пути из `src/content/`**: там лежат только статьи блога, и slug вроде `oracle-strategy`, `risk-management` — норма, а не внутренние документы.
