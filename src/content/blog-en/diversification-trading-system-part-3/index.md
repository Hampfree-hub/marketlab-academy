...
In this part — tuning for yourself and what to avoid. Previous parts: [Part 1](https://marketlab-academy.org/en/library/diversification-trading-system-part-1/) (why diversify), [Part 2](https://marketlab-academy.org/en/library/diversification-trading-system-part-2/) (how to build rules and configure bots).
...
For systematic trade tracking, it's recommended to keep a trading journal. This can be a paper notebook, a spreadsheet, or a specialized platform. At Market Lab, we use [TMM](https://marketlab-academy.org/en/library/trading-journal-guide/) for automatic statistics collection and trade analysis.

## When and how to tune

Don't change rules after one trade: one drawdown or one profit is not a reason to rewrite the system. Tuning — after a period (week, month): if the system is steadily in the red or drawdown exceeds what you accept, then revising parameters is justified. Before changing settings, test the new configuration on history — [backtests](https://marketlab-academy.org/en/library/what-are-backtests/) show how the system would have behaved in the past. Otherwise you risk fitting to past data and overfitting: nice on history, drawdown again in live.
...
- **No diversification** — all volume in one asset, one bot, or one exchange; one failure hits all capital. See [Part 1](https://marketlab-academy.org/en/library/diversification-trading-system-part-1/).
...
- Venues checked: uptime, withdrawal limits, reputation — see [exchange uptime and downtime](https://marketlab-academy.org/en/library/exchange-uptime-downtime/).
...
- [Risk Management in Crypto Trading](https://marketlab-academy.org/en/library/risk-management-crypto-trading-bots/) — capital protection principles
- [Trading Journal](https://marketlab-academy.org/en/library/trading-journal-guide/) — how to keep trade records
- [DCA Strategy](https://marketlab-academy.org/en/library/dca-strategy-crypto/) — time-based entry averaging
- [Grid Trading](https://marketlab-academy.org/en/library/grid-trading-crypto/) — range-based trading
- [Exchange Uptime](https://marketlab-academy.org/en/library/exchange-uptime-downtime/) — choosing a reliable venue
