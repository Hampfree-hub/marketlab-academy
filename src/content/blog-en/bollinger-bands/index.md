...
Bollinger Bands are a volatility indicator: a middle line (moving average) and two bands above and below, whose distance depends on the standard deviation of price. The tool helps gauge the "range" of movement and look for bounces off the channel boundaries. This article covers how the bands are calculated, how to read touches of the upper and lower band, and how to combine them with [RSI](https://marketlab-academy.org/en/library/technical-analysis-rsi/) and [Williams %R](https://marketlab-academy.org/en/library/williams-percent-range-r/).
...
The indicator was developed by John Bollinger in the 1980s. The idea is that price spends most of the time inside the bands, and a break or touch of the bands often precedes a reversal or correction. In technical analysis and [trading signals](https://marketlab-academy.org/en/library/what-are-trading-signals/) articles, Bollinger Bands are grouped with volatility indicators such as [ATR](https://marketlab-academy.org/en/library/atr-and-atr-percent/).
...
Typical settings are 20 periods and multiplier 2. On lower [timeframes](https://marketlab-academy.org/en/library/timeframes-trading/) a shorter period (e.g. 15) is sometimes used so the indicator reacts faster.
...
Important: in a strong trend, price can "walk the band" — stay near the upper boundary in an uptrend or the lower one in a downtrend. In those conditions, touching the band is not a standalone reversal signal. Confirmation is better sought from [RSI](https://marketlab-academy.org/en/library/technical-analysis-rsi/) or [Williams %R](https://marketlab-academy.org/en/library/williams-percent-range-r/) and support/resistance levels.
...
- **Potential buy:** price touches or breaks the lower band, then reverses and closes back inside the bands. Confirmation can be [RSI](https://marketlab-academy.org/en/library/technical-analysis-rsi/) bouncing from oversold or [Williams %R](https://marketlab-academy.org/en/library/williams-percent-range-r/) leaving the zone below −80.
...
One indicator can give false signals: e.g. price at the lower band but the trend still down. Combining Bollinger Bands with oscillators and, when useful, [sideways trend strategies](https://marketlab-academy.org/en/library/sideways-trend-strategies/) improves filtering.
...
- **Bollinger Bands + Williams %R:** as in the [Williams %R](https://marketlab-academy.org/en/library/williams-percent-range-r/) article, %R and the bands together help spot when price has moved far from the average and the oscillator confirms the extreme.
- **Bollinger Bands + volume:** a volume spike on a bounce from the lower or upper band can confirm the strength of the reversal.

Avoid cluttering the chart: 2–3 indicators are usually enough. More on combinations in [trading signals](https://marketlab-academy.org/en/library/what-are-trading-signals/) and [backtests](https://marketlab-academy.org/en/library/what-are-backtests/).

## Use in Crypto Trading

The crypto market is highly volatile. Bands on crypto pairs often widen a lot on sharp moves and narrow in flat markets. Default 20 and 2 can work; when scalping on [short timeframes](https://marketlab-academy.org/en/library/timeframes-trading/) a shorter period is sometimes used. It pays to test settings on history with [backtests](https://marketlab-academy.org/en/library/what-are-backtests/) for the chosen asset and style.
...
- Combining with [RSI](https://marketlab-academy.org/en/library/technical-analysis-rsi/), [Williams %R](https://marketlab-academy.org/en/library/williams-percent-range-r/) and, when needed, volume improves signal filtering.
