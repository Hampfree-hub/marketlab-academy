...
**Algorithmic trading** — using predefined rules (algorithms) to buy and sell assets. Come to think of it, when a caveman picked up a stick and rolled a huge boulder off the path — that's also an algorithm: "If rock blocks path → pick up stick → roll rock."
...
**Algorithm** — step-by-step instructions to achieve a goal.

**Real-life examples:**

1. **Borscht recipe:**
   - If you have beets → grate them
   - If no meat → add beans
   - Cook for 2 hours → serve with sour cream

2. **Getting ready for work:**
   - If alarm rings → get up
   - If raining → take umbrella
   - If traffic jam → turn on navigator

**Trading algorithm:**
- If price crosses moving average from below → buy
- If price drops 5% from high → sell
- If time is 18:00 → close all positions
...
**His algorithm:**
- If stock rises 3 days in a row → buy
- If drops 10% from peak → sell
- If news is bad → reduce position
...
- Simple conditions: "If MA(50) > MA(200) → buy"

**Problem:** Room-sized computers, expensive, slow.

**1971:** NASDAQ — first electronic quotation system.
...
- 24/7 (no weekends)
- High volatility (±10% per day — normal)
...
**What AI can do:**
- Analyze news (NLP — natural language processing)
- Recognize patterns on charts (CNN — convolutional networks)
- Optimize strategy parameters (RL — reinforcement learning)
...
**Algorithm:**
- If price above MA(50) and MA(200) → long
- If price below MA(50) and MA(200) → short
- If MAs cross → exit
...
- Spread: $200
- If both sides executed → profit $200
...
**Cons:**
- ❌ Doesn't maximize profit (could have bought at the bottom)
- ❌ Requires discipline
...
- If price oscillates → profit on each cycle

**Example:**
- Price: $95,000 → $96,000 → $95,000 → $96,000
- Bought at $95k, sold at $96k → +$1,000
- Repeated 10 times → +$10,000

**Pros:**
- ✅ Works in sideways (70% of the time)
- ✅ Automatic, 24/7

**Cons:**
- ❌ Dangerous in trend (price exited range)
- ❌ Requires large capital (many orders)
- ⚠️ **Important:** If price exits the range, the bot may end up with a full bag of losing assets (bought everything, didn't sell).
...
```
IF [entry condition] → THEN [action]
IF [exit condition] → THEN [action]
IF [risk condition] → THEN [action]
```

**Example (trend):**

```
IF price > MA(50) AND MA(50) > MA(200) → BUY
IF price < MA(50) → SELL
IF loss > 5% → STOP-LOSS
```
...
**Ready Platforms:**
- **Veles:** Crypto algo trading, backtests, cloud bots — [try Veles](https://veles.finance/invite/washmallay) *(affiliate link)*
...
- If you change parameters to fit history → algorithm won't work on new data
- Better a simple algorithm than an overfitted one

**Keeping a journal:**

Alongside the algorithm, it's recommended to keep a [trading journal](https://marketlab-academy.org/en/library/trading-journal-guide/) — record why you entered a trade, why you exited, what emotions you felt. This helps identify behavioral patterns and improve discipline.
...
**Algorithmic trading** — using rules for automated trading. From "caveman's stick" to AI bots.
...
- ✅ Traders who want to automate routine
- ✅ Investors for DCA and rebalancing
- ✅ Developers who love code + finance

**Who it's NOT for:**
- ❌ Expecting a "money button" (algorithm is a tool, not magic)
- ❌ Not ready to learn (code, statistics, risks)
- ❌ Want 100% guarantees (they don't exist even with algorithm)
...
No more than 1-2% of capital per trade. Maximum drawdown — 20% (after that, review strategy).
